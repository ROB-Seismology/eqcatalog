...
c
      ...
      integer*2 jcent, icent, icent0, icent1, icent2
...
C-- GET TO WORK
      ...
      n=0
      nclust=0
      open (2,file='cluster.out',status='unknown',action='write')
      open (7,file='cluster.ano',status='unknown',action='write')
      open (8,file='cluster.dec',status='unknown',action='write')
      open (9,file='cluster.clu',status='unknown',action='write')
...
C Write out cluster summary to "output" file
  ...
  510 write (2,511) ichr,k,(ctim0(it,k),it=1,5),cdur(k),nc(k),lat,
     1       xlat,lon,xlon,cdep(k),xmag,icent1,(ctim1(it,k),it=1,5),
     2       cmag1(k),icent2,(ctim2(it,k),it=1,5),cmag2(k),fore,DT,DM
...
c---- t1b is number of days from 00:00 1/1/69 to 00:00 1/1/itime(1)
      t1b = (itime(1)-1969)*365 + int((itime(1)-1969.)/4.)
...
c---- t2b is number of days from 00:00 1/1/69 to 00:00 1/1/jtime(1)
      t2b = (jtime(1)-1969)*365 + int((jtime(1)-1969.)/4.)
